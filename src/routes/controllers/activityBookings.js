...
export default ({ api, db }) => {
    ...
    jwt
      ...
      .then(activity => {
        return res.status(200).json({
          error: null,
          data: activity.bookings.filter(e => e.isConfirmed)
        });
      })
      //return errors
      .catch(error => {
  ...
  api.get('/activities/:activityId/bookings/:bookingId', (req, res) => {
    ...
    jwt
      .verify(req)
      .then(() =>
        db.getActivityBookingById(req.params.activityId, req.params.bookingId)
      )
      .then(booking => {
  ...
  api.put('/activities/:activityId/bookings/:bookingId', (req, res) => {
    ...
    jwt
      .verify(req)
      .then(token =>
        db.isRightfulActivityOwner(token.username, req.params.activityId)
      )
      .then(() =>
        db.updateActivityBookingById(
  ...
  api.post('/activities/:activityId/booking-requests', (req, res) => {
    jwt
      ...
      .then(() =>
        db.insertBooking(
          req.params.activityId,
          _.extend(req.body, { isConfirmed: false })
        )
      )
      .then(booking => {
        return res.status(200).json({ error: null, data: booking });
  ...
  api.put(
    '/activities/:activityId/booking-requests/:bookingId/verify',
    (req, res) => {
      jwt
        .verify(req)
        .then(token =>
        ...
        .then(() =>
          db.confirmBooking(req.params.activityId, req.params.bookingId)
        )
        .then(booking => {
          return res.status(200).json({ error: null, data: booking });
        })
