...
export default ({ api, db }) => {
  ...
  api.get('/activity-types/addition-requests', (req, res) => {
    jwt
      ...
      .then(activityTypes => {
        return res.status(200).json({ error: null, data: activityTypes });
      })
      .catch(error => {
        return res
          .status(error.status)
  ...
  api.post('/activity-types/addition-requests', (req, res) => {
    jwt
      .verify(req)
      .then(() =>
        db.insertOneActivityType(_.extend(req.body, { isConfirmed: false }))
      )
      .then(activityType => {
        return res.status(200).json({ error: null, data: activityType });
      })
      .catch(error => {
        return res
          .status(error.status)
  ...
  api.put('/activity-types/addition-requests/:id/verify', (req, res) => {
    jwt
      ...
      .then(() =>
        db.updateActivityTypeById(req.params.id, { isConfirmed: true })
      )
      .then(activityType => {
        return res.status(200).json({ error: null, data: activityType });
      })
  ...
  api.post('/activity-types', (req, res) => {
    jwt
      ...
      .then(token => jwt.isAdmin(token))
      .then(() =>
        db.insertOneActivityType(_.extend(req.body, { isConfirmed: true }))
      )
      .then(activityType => {
        return res.status(200).json({ error: null, data: activityType });
      })
