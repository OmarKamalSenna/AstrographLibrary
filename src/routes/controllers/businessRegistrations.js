...
export default ({ api, db }) => {
  ...
  api.get('/business-registrations', (req, res, next) => {
    ...
    jwt
      ...
      .catch(() =>
        res
          .status(401)
          .json({ error: errors.INVALID_TOKEN.message, data: null })
      )
      // Check if admin
      .then(token => jwt.isAdmin(token))
      // Not an admin
      .catch(() =>
        res.status(403).json({ error: errors.NOT_ADMIN.message, data: null })
      )
      // Get resource
      ...
      .catch(() =>
        res
          .status(500)
          .json({ error: errors.INTERNAL_SERVER_ERROR.message, data: null })
      )
      ...
      .then(collection => {
        if (_.isEmpty(collection)) {
          ...
          return res
            .status(404)
            .json({
              error: errors.ENTITY_NOT_FOUND.message,
              data: { collection }
            });
        }
        // Resource found
  ...
  api.post('/business-registrations', (req, res, next) => {
    ...
    jwt
      ...
      // Not a client
      .catch(() =>
        res.status(403).json({ error: errors.NOT_CLIENT.message, data: null })
      )
      // Create resource
      ...
      .then(document =>
        res.status(201).json({ error: null, data: { document } })
      )
      // Done
      .catch(() => next());
  });
  ...
  api.put('/business-registrations/:id/verify', (req, res, next) => {
    ...
    jwt
      ...
      // Update resource
      .then(() =>
        db.updateBusinessRegistrationById(req.params.id, { isVerified: true })
      )
